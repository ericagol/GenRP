...
fix,axes = subplots(1,4,figsize=[20,4])
#fix,ax = subplots()

ax = axes[1]
ax[:set_title]("(a). Simple, exact kernels",fontsize=2*fontsize)
...
ax[:set_xlabel](L"$\tau/\tau_0$",fontsize=2*fontsize)
ax[:set_ylabel]("Dimensional ACF",fontsize=2*fontsize)
ax[:axis]([0,4pi,-1.1,1.1])

ax = axes[3]
ax[:set_title]("(b). Gaussian",fontsize=2*fontsize)
...
ax[:set_title]("(c). Exp-sine-squared",fontsize=2*fontsize)
...
ax[:set_title]("(d). Matern",fontsize=2*fontsize)
ax[:legend](loc="upper right",fontsize=fontsize)
ax[:set_xlabel](L"$\tau/\tau_0$",fontsize=2*fontsize)
#ax[:set_ylabel]("Dimensional ACF",fontsize=2*fontsize)
ax[:axis]([0,pi,0,1.1])
